document.addEventListener("DOMContentLoaded", () => {
  ...
  const FORMSPREE_URL = "FORMSPREE_ID_PROD";
  ...
  const SANITY_PROJECT_ID = "ID_SANITY_PROD";
  const SANITY_DATASET = "production";
  const SANITY_QUERY = encodeURIComponent(
    '*[_type == "temoignage"] | order(_createdAt desc)',
  );
  ...
  function updateTooltipPosition() {
    ...
    tooltip.textContent = DEBUG_MODE
      ? `Test : ${formatTime(rem)}`
      : `Attendez encore ${formatTime(rem)}`;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      showConfirmation(
        "Témoignage envoyé ! Il sera ajouté après vérification.",
      );
      updateButtonState();
    } catch (err) {
  ...
  // Chargement des témoignages depuis Sanity et initialisation de l'affichage
  updateButtonState();
  (async function loadTestimonials() {
    try {
      const r = await fetch(SANITY_URL);
      if (!r.ok) throw new Error("Erreur API Sanity");
      const json = await r.json();
      testimonials = json.result;
